...
class LoginController extends Controller
{
    ...
    public function reset_password(){
        $asesor =  Auth::guard(config('overrides.guard'))->user();
    ...
    public function reset_password_admin(Request $request){
        DB::table(config('overrides.table'))
        ->where('id', $request->input('id'))
        ->update(values: ['reset' => true,'password' => bcrypt(config('overrides.default_password'))]);
    }
